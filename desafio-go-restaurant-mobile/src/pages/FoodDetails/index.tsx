...
const FoodDetails: React.FC = () => {
  ...
  async function handleFinishOrder(): Promise<void> {
    ...
    const foodAlreadyOrdered = ordersResponse.data.filter((order: Food) => {
      return order.id === food.id;
    });

    if (foodAlreadyOrdered.length > 0) {
      Alert.alert('Este item já foi adicionado ao pedido.');
      return;
    }

    const foodDraft = food;

    delete foodDraft.formattedPrice;

    await api.post('/orders', { ...foodDraft });
